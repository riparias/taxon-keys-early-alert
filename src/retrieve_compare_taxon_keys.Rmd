...
```{r read_species_list}
input_data <- read_csv(
  file = here("data", "reference", "Species-2023-04-25.csv")
) 
```
...
If a synonym occurs in the RIPARIAS target species list, we will use the correspondent accepted taxon key from the backbone.
...
Sometimes we need to refer to different taxa than the ones harvested via checklists.

The species _Zizania latifolia_ is matched to the wrong taxon. We suspect a "bug" at GBIF side: see [issue](https://github.com/gbif/portal-feedback/issues/4718).

We retrieve the correct match:

```{r find_correct_match_zizania}
zizania_latifolia <- rgbif::name_backbone(name = "Zizania latifolia")
zizania_latifolia
```

and replace the wrong match in `taxa_checklists`:

```{r replace_zizania_lat}
taxa_checklists %>%
  mutate(nubKey = if_else(species != "Zizania latifolia",
                          nubKey,
                          zizania_latifolia$usageKey)
  )
```

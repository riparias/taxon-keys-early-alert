...
We harvest taxonomic information from the backbone, as these keys are the ones linked to the occurrences and so used in the early alert tool. Notice that some checklists define synonymy relationships, see columns `acceptedKey` and `accepted`. We will search the match to GBIF Backbone for the accepted taxa as well:
...
taxa <- purrr::map_df(
  ...
  distinct()
extra_taxa <- purrr::map_df(
  taxa_checklists$acceptedKey, 
  function(x) {
    if (!is.na(x)) {
      taxa_info <- rgbif::name_usage(x)$data
    }
  ...
  distinct()
# Get all taxa information from the backbone
taxa <- bind_rows(taxa, extra_taxa) %>%
  distinct()
```
...
# Add accepted keys for synonyms if any
if (!is.null(accepted)) {
...
## Explore matches and mismatches

This section is good for finding mismatches between the names in the list used in the early alert tool and the GBIF Backbone, which can be due to misspellings, taxonomic updates, or other issues.

```{r}
dplyr::setdiff(alert_species$canonicalName,input_data$name)
```

```{r}
dplyr::setdiff(alert_species$key, input_data$gbif_taxon_key)
```

Are there species without match to the backbone?

```{r}
input_data %>%
  filter(!gbif_taxon_key %in% alert_species$key)
```
